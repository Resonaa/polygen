...
function GameMessage({
  ...
  content,
  sender
}: Pick<Message, "type" | "content" | "sender">) {
  return (
    <div>
      <a
        href={`/user/${sender}`}
        style={{ color: "light" + getColorByMessageType(type) }}
      >
        {sender}
      </a>
      <RenderedText
...
export function Messages({ client }: { client?: ClientSocket }) {
    ...
    setPreviousTime(previousTime => {
      ...
      if (deltaTime >= 1000 * 60 * 5) {
        ...
        setMessages(messages =>
          messages.concat([
            <Time key={messages.length} time={curTime} />,
        ...
        setMessages(messages =>
          messages.concat(<Fragment key={messages.length}>{content}</Fragment>)
        );
        return previousTime;
      }
    });
  ...
  useEffect(() => {
    ...
    client
      .on("message", ({ sender, content, type }) =>
        updateMessageList(
          <GameMessage content={content} type={type} sender={sender} />
        )
      )
      .on("info", info => updateMessageList(<Info content={info} />))
      .on("disconnect", () => updateMessageList(<Info content={"连接断开"} />))
      .on("win", winnerStr =>
        updateMessageList(<Info content={winnerStr + "赢了"} />)
      );
  }, [client]);
