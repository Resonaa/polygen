...
export function Vote({ client, voteData, type, setType }: {
  client?: ClientSocket,
  ...
  function Item<T extends VoteItem>({ value, players }: { value: VoteValue<T>, players?: string[] }) {
    return (
      <List.Item active={players && players.includes(user.username)} onClick={() => client?.emit("vote", {
        item: type as VoteItem,
        value
      })} className="select-none">
        <List.Content>
          <List.Header>{value}</List.Header>
          {players && <>{players.length}票{players.length > 0 ? ": " : ""}{players.join(", ")}</>}
        </List.Content>
      </List.Item>
    );
  }
    ...
    >
      ...
      <Modal.Content scrolling>
        <Modal.Description>
          ...
          <List divided inverted relaxed selection>
          ...
          <List divided inverted relaxed selection>
            {(() => {
              ...
              const data = voteData.data[type] ? voteData.data[type] as NonNullable<VoteData[typeof type]> : [];
              const others = (voteItems[type] as Array<VoteValue<typeof type>>)
                .filter(value => !data.some(([existingValue,]) => existingValue === value));
