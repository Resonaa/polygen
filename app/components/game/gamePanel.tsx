import { Button, Center, chakra, Heading, HStack, SimpleGrid, useColorModeValue } from "@chakra-ui/react";
...
function PlayerList({ players, readyPlayers }: { players: string[], readyPlayers: string[] }) {
  return (
    <HStack gap={2} mt={2}>
      {players.map(username => <UserTag username={username} key={username}
                                        textDecoration={readyPlayers.includes(username) ? "underline" : undefined} />)}
    </HStack>
  );
}
...
function TeamCard(
  { title, children, disabled, onClick }:
    { title: string, children?: ReactNode, disabled: boolean, onClick: () => void }
) {
  return (
    <Button as={Center} flexDir="column" h="unset"
            p={2} cursor={disabled ? "unset" : "pointer"} onClick={disabled ? undefined : onClick} variant="outline">
...
export default function GamePanel({ client, rid }: { client?: ClientSocket, rid: string }) {
  ...
  const canCreateTeam = teamData.some(([team, players]) =>
      ...
      <Center flexDir="column" gap={4} h="100%" hidden={showCanvas}>
        ...
        <SimpleGrid gap={3} w="100%" minChildWidth="120px">
          {teamData.map(([team, players]) => {
            ...
            return (
              <TeamCard key={team} title={title} disabled={disabled} onClick={() => client?.emit("joinTeam", team)}>
          ...
          <TeamCard title="创建新队伍" disabled={!canCreateTeam} onClick={() => client?.emit("joinTeam", undefined)} />
        </SimpleGrid>

        <Button w="100%" isDisabled={isSpectating} onClick={event => {
          client?.emit("ready");
